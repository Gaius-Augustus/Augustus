/*
 * Name:    pp_simscore.cc
 * Project: Similarity-Score algorithm for Protein-Profile and Protein-Sequence
 * Author:  Lars Gabriel
 *
 */
...
using namespace SS;	
    ...
    //initiation of a row with its length and the position of the first element in the matrix
    ...
    //the file has to be in FASTA format and it can have an additional section with [intron position]
    ...
    char& ProteinSequence::operator[] (int n) {
        ...
        throw string("Protein sequence out of range.");
    }
    ...
    //returns the number of residual nucleotides f, if an intron is in the protein sequence at (j,f) located
    ...
    int ProteinSequence::intronsInRange (int start, int end) {
        if (start == 0)
            start =-1;
        std::map < int, int >::iterator s = introns.upper_bound(start-1); 
        std::map < int, int >::iterator e = introns.lower_bound(end);
        return std::distance(s, e);
        
    }

    //intiates parameter
    SimilarityScore::SimilarityScore (double g, double b, double g_i, double iw1, double iw2, double e_i, double e_n) {
        gap_cost_inter = g;
        gap_cost_intra = b;
        gap_cost_intron = g_i;
    ...
    void SimilarityScore::readFiles (const char* seqFileName, const char* profileFileName) {
    ...
    void SimilarityScore::fillSimilarityMatrix () {
        ...
        //k_max: position of the last cell, which is used to compute the score of a cell, if i represents the first position of a new block
        ...
        //col holds the current block column that is represented by the current row of the similarity matrix
	    PP::Column col; 
	    //d holds the inter-block distnace interval that is located before the current block of the profile
	    PP::DistanceType d;	
	    ...
	    //the algorithm only computes, if the protein sequence is longer than the number of block columsn in the protein profile
	    if (seq->length() < min_prfl_length) {
	        std::cerr<<"protein sequence is to short"<<endl;
	        exit(1);	        
	    }
	    else {
	    //constraint for the similarity score algorithm
	    //at any point of the algorithm the whole protein-profile must be able to be matched  
	    row_length = seq->length()-min_prfl_length+1;
	    
	    intronPrfl_noProt = prfl -> getIntronNoSeq();
                ...
                old_score = std::numeric_limits<int>::min();
                    ...
                    for (int k = 0; k < k_max; k++) {
                        ...
                        if(intron_weight_inter>0) {
                            noIntronsSeq = seq->intronsInRange(S[i-1].position + k-1, position + j -1);
                            ...
                            if (noIntronsSeq == 0 && intronFreqPos < 0) {
                                ...
                                intronFreqPos = (intronFreqPos + epsi_intron)/(intronPrfl_noProt + epsi_intron + epsi_noIntron);
                                ...
                                if (interblock_length < 1) {
                                    ...
                                    //intron match
                                    iscore = intronScore(intronFreqPos, PoiDist(noIntronsSeq, (d.r.min+d.r.max+ 2)/2.0 * (intronAvgInterBlock_bfreq/intronPrfl_noProt) * 3));
                                }
                            }
                        }                        
                        //calculate score
                        min_gaps = 0;
                        type = 'm';
                        //calculate the minimal gaps needed for this inter-block section
                        ...
                        if (k == diff_pos) {
                            ...
                            new_score = S[i-1][k].score + gap_cost_inter * min_gaps + gap_cost_intra;
                            type = 's';
                        }
                        else {
                            //first block column of B_t as an intra-block gap
                            match_score = col.L(GeneticCode::get_aa_from_symbol((*seq)[position+j-1]));
                            if (gap_cost_inter > match_score) {
                                ...
                                new_score = S[i-1][k].score + gap_cost_inter * min_gaps + match_score;                                
                            }
                        }
                        old_score = compareScores(new_score + iscore, old_score, i, j, i-1, k, type);
                    }
                    if (j > 0) {
                    //intronscore
                        //gap in profile
                        ...
                        if (frame < 0) {
                            ...
                            iscore = intronScore((epsi_intron/epsi_intron + epsi_noIntron),3*intronIntraBlock_bfreq);
                        }
        ...
        for (int j=0; j<row_length; j++) {
            ...
            iscore = 0;
            if(intron_weight_inter>0) {  
                noIntronsSeq = seq->intronsInRange (seq->length() - diff_pos + j, seq->length());
    ...
    //computes the intra-block intron score for intron positions (k,s,f) in the profile and the aligned position in P
    //intron_frame denotes the number of residual nucleotides in P, after which an intron occurs or -1 if there no intron is in P located
    double SimilarityScore::intraBlock_iscore (int k, int s, int f, int intron_frame) {
        ...
        if (intron_frame == f) {
            return intron_weight_intra * (log10((prfl_freq + epsi_intron)/(intronPrfl_noProt + epsi_intron + epsi_noIntron)) - log10(intronIntraBlock_bfreq));
        }
        else if (intron_frame != -1 && prfl_freq > 0) {
            return intron_weight_intra * (log10(1-((prfl_freq + epsi_intron)/(intronPrfl_noProt + epsi_intron + epsi_noIntron))) - log10(1-intronIntraBlock_bfreq));
        }
        else if (prfl_freq > 0) {
            return intron_weight_intra * log10(1-((prfl_freq + epsi_intron)/(intronPrfl_noProt + epsi_intron + epsi_noIntron)));
        }
        else
            return 0;
    }    
    ...
    //this method is used for the algorithm, which fills the similarity matrix
    ...
    void SimilarityScore::backtrackingDB (int number_Alignments) {
        //stores for every alignment the connected regions of the same alignment type in a block of the protein profile
        //for this the start coordinates in the protein profile (block number, column number) and the protein sequence, the length and the type for every region is stored
        int k_max;
        if (S.empty()) {
            std::cerr<<"Similarity Matrix is empty"<<endl;
            exit(3);
        }
        ...
        if (number_Alignments > 0) {
            ...
            //initial case for the recursion: start a new alignemnt for every predecessor of the final similarity score
            k_max = (number_Alignments < S[new_bt.i][new_bt.j].prev.size()) ? number_Alignments : S[new_bt.i][new_bt.j].prev.size();
            for (int k = k_max-1; k > -1; k=k-1) {
            ...
            while (!bt.empty()) {
                ...
                for (int t = 0; t < bt_size; t++) {
                    k_max = (number_Alignments < S[bt[t].i][bt[t].j].prev.size()) ? number_Alignments : S[bt[t].i][bt[t].j].prev.size();
                    ...
                    for (int k = k_max - 1; k > -1; k=k-1) {
                        ...
                        if (new_bt.i < S.length() - 2 && (new_bt.align_element.type != 'p' || new_bt.i < S.length() - 3)){ 
                            //add new Element to current alignment, if the new element type is not equal to the last element type or a block is finished
                            if (new_bt.align_element.type != new_type || (new_bt.align_element.col_pos == 0 && new_bt.align_element.type != 'p')) {
                        ...
                        new_bt.align_element.length++;// = new_bt.col_posStart - new_bt.col_count + 1;

                        if (new_type == 's' || new_type == '_' || (new_type=='m' && new_bt.i < S.length()-1)) {
                            new_bt.col_count = new_bt.col_count - 1;
                        }
                        //update current position in the similarity matrix and corresponding position in the protein profile
                        ...
                        //create new alignments, if there is more than one predecessor in the similarty matrix
                        else if (k > 0) {
                            bt.push_back(new_bt);
                        }
    ...
    void SimilarityScore::printAlignmentDB () {
        for (int a=0; a<alignments.size(); a++) {
            for (int i =0; i<alignments[0].size(); i++) {
                cout<<a<<'\t'<<i<<'\t'<<alignments[a][i].protSeq_pos<<'\t'<<alignments[a][i].block_pos<<'\t'<<alignments[a][i].col_pos<<'\t'<<alignments[a][i].length<<'\t'<<alignments[a][i].type<<endl;
            }
        }
    }
    ...
    void SimilarityScore::printAlignment () { 
        ...
        if (alignments.empty()) {
            std::cerr<<"There are no alignemnts"<<endl;
            exit(4);
        }
        ...
        for (int i = 0; i < alignments.size(); i++) {
            ...
            for (int j = 0; j < alignments[i].size(); j++) {
                ...
                if (j==alignments[i].size()-1) {
                    ...
                    prfl_align.argmax += to_string( seq->length() - pos);
                }
            }
        ...
        for (int i = 0; i < alignmentsReadable.size(); i++) {
            ...
            for (int k = 0; k < row_numb; k++) {
                ...
                std::cout << alignmentsReadable[i].second.argmax.substr(k * char_numb, char_numb);
                std::cout<<endl;
                ...
                for (int d = 1; d < number_of_decimals+1; d++) {
                    ...
                    if  (d == 2) {
                        ...
                        std::cout << std::left << std::setw(25) << "of prot. seq. character:";
                    }
                    else {
                    ...
                    while (prot_index < (k+1)*char_numb && prob_index[d-1] < alignmentsReadable[i].second.match_prob.size()) {
                        if (std::isdigit(alignmentsReadable[i].first[prot_index]) || alignmentsReadable[i].first[prot_index] == '_' || alignmentsReadable[i].first[prot_index] == 's' || alignmentsReadable[i].second.argmax[prot_index] == '_') {
                            std::cout<<' ';
                        }
                        else {
...
{
    //TODO  option for how many alignments should be printed
    //      option for which proteins should be used from the data base
    //      option for std out for db
    ...
    const option long_opts[] = {
        ...
        {"out", required_argument, nullptr, 'o'},        //{"db", required_argument, nullptr, 'd'},
    ...
    if (!fasta.empty() && !prfl.empty()) {
        SimilarityScore SS(gapCost_inter, gapCost_intra, gapCost_intron, intron_weight_intra, intron_weight_inter, epsilon_intron, epsilon_noIntron);
