...
//read an intron profile for a block and the inter-block section before the block
IntronProfile::IntronProfile (const vector<string>& lines) {
    int col, nucleo, numIntron;
    int freq;
    double d;
    for (unsigned lineno=0; lineno < lines.size(); lineno++) {
        if (lines[lineno] == "")
            continue;
        if (istringstream(lines[lineno]) >> col >> nucleo >> freq) {
            if (0 <= col && 0 <= nucleo && nucleo < 3 && 0 <= freq && freq <= numSeq)
                mapIntraBlock.insert(make_pair(make_pair(col,nucleo), freq));
            else
                throw PartParseError(lines.size()-lineno);
        }
        else if (istringstream(lines[lineno]) >> numIntron >> freq) {
            if (0 <= numIntron && 0 <= freq && freq <= numSeq)
                mapInterBlock.insert(make_pair(numIntron, freq));
            else
            	throw PartParseError(lines.size()-lineno);
        }
...
void Profile::parse_stream(istream & strm) {
    ...
    //Author: Lars Gabriel
    if (type == "[intron profile]")
        iP.insert(make_pair(blocks.size(), IntronProfile(readPart(strm, type, lineno))));
    if (type != "")
