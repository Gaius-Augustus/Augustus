 ...
 * gaplesse fragments exceed this lenght threshold.
...
void capAliSize(list<Alignment*> &alis, int maxRange){
	...
	if ((*ait)->maxRange() > maxRange){
	    Alignment *a = *ait;
	    //cout << "splitting up alignment " << *a << endl;
		...
		if (a->rows[s]->frags[bidx[s]].chrEnd() - a->rows[s]->frags[0].chrPos + 1 > maxRange){
			...
			if (a->rows[ss]){
				...
				//delete a->rows[ss]; for some reason this causes an error later, even when set to NULL?!
				//cout << "deleting row " << ss << " " << *(a->rows[ss]) << endl << flush;
				//a->rows[ss] = NULL;
			    }
			    // update offset as minimum of all remaining alignment positions
			    if (a->rows[ss]->frags.size() > 0 && offset > a->rows[ss]->frags[0].aliPos)
		    ...
		    *ait = newAli;
			...
			if (seqlen > maxSeqLen)
			    maxSeqLen = seqlen;
			if (gapsize > maxgapsize && // the cut is made at the best place in the second half
			    maxSeqLen >= 2*maxRange/3 && (maxSeqLen <= maxRange || maxgapsize < 0)){
			    maxgapsize = gapsize;
			    curbestidx = bidx;
			}
