...
class TrainingController {
	...
	def beforeInterceptor = {
		...
		qstatFile << "/usr/bin/qstat | grep qw | wc -l > ${output_dir}/${qstatFilePrefix}.qstatResult"
		...
		if(!(trainingInstance.id == null)){
			...
			if(!uploadedGenomeFile.empty){
				if(preUploadSize <= maxButtonFileSize){
					...
					if("${uploadedGenomeFile.originalFilename}" =~ /\.gz/){
						...
						gunzipGenomeScript << "cd ${projectDir}; mv genome.fa genome.fa.gz; gunzip genome.fa.gz 2> ${projectDir}/genomegunzip.err"
					...
					if(genomeFastaFlag == 1) {
						...
						genomeCksumScript << "cksum ${projectDir}/genome.fa > ${genomeCksumFile} 2> ${projectDir}/genome.cksum.err"
						...
						//////delProc = "rm ${projectDir}/genome.cksum".execute()
						//////delProc.waitFor()
						//////delProc = "rm ${projectDir}/genome_cksum.sh".execute()
						//////delProc.waitFor()
					}
				}else{
						logFile <<  "${trainingInstance.accession_id} The selected genome file was bigger than ${maxButtonFileSize}. Submission rejected.\n"
			...
			if(!uploadedEstFile.empty){
				if(preUploadSize <= maxButtonFileSize){
					...
					if("${uploadedEstFile.originalFilename}" =~ /\.gz/){
						...
						gunzipEstScript << "cd ${projectDir};\n mv est.fa est.fa.gz;\n gunzip est.fa.gz 2> ${projectDir}/est.gunzip.err\n"
						def gunzipEst = "bash ${projectDir}/gunzipEst.sh".execute()
						gunzipEst.waitFor()
						logFile <<  "${trainingInstance.accession_id} Unpacked EST file.\n"
						//////delProc = "rm ${projectDir}/gunzipEst.sh".execute()
						//////delProc.waitFor()
					}
					...
					estCksumScript << "cksum ${projectDir}/est.fa > ${estCksumFile} 2> ${projectDir}/est.cksum.err"
					...
					//////delProc = "rm ${projectDir}/est.cksum".execute()
					//////delProc.waitFor()
					//////delProc = "rm ${projectDir}/est_cksum.sh".execute()
					//////delProc.waitFor()
				}else{
						logFile <<  "${trainingInstance.accession_id} The selected cDNA file was bigger than ${maxButtonFileSize}. Submission rejected.\n"
			...
			if(!uploadedStructFile.empty){
				if(preUploadSize <= maxButtonFileSize * 2){
					...
					logFile <<  "${trainingInstance.accession_id} The selected training gene structure file was bigger than ${allowedStructSize}. Submission rejected.\n"
				...
				structCksumScript << "cksum ${projectDir}/training-gene-structure.gff > ${structCksumFile} 2> ${projectDir}/struct.cksum.err"
				...
				//////delProc = "rm ${projectDir}/struct.cksum".execute()
				//////delProc.waitFor()
				//////delProc = "rm ${projectDir}/struct_cksum.sh".execute()
				//////delProc.waitFor()
			}
			...
			if(!uploadedProteinFile.empty){
				if(preUploadSize <= maxButtonFileSize){
					...
					logFile <<  "${trainingInstance.accession_id} The selected protein file was bigger than ${maxButtonFileSize}. Submission rejected.\n"
				...
				if("${uploadedProteinFile.originalFilename}" =~ /\.gz/){
					...
					gunzipProteinScript << "cd ${projectDir}; mv protein.fa protein.fa.gz; gunzip protein.fa.gz 2> ${projectDir}/protein.gunzip.err"
					...
					//////delProc = "rm ${gunzipProteinScript}".execute()
					//////delProc.waitFor()
				}
				...
				proteinCksumScript << "cksum ${projectDir}/protein.fa > ${proteinCksumFile} 2> ${projectDir}/protein.cksum.err"
				...
				//////delProc = "rm ${projectDir}/protein.cksum".execute()
				//////delProc.waitFor()
				//////delProc = "rm ${projectDir}/protein_cksum.sh".execute()
				//////delProc.waitFor()
			}
			...
			if(!trainingInstance.hasErrors() && trainingInstance.save()){
				...
				emptyPageScript << "${AUGUSTUS_SCRIPTS_PATH}/writeResultsPage.pl ${trainingInstance.accession_id} ${trainingInstance.project_name} ${dbFile} ${output_dir} ${web_output_dir} ${AUGUSTUS_CONFIG_PATH} ${AUGUSTUS_SCRIPTS_PATH} 0 2> ${projectDir}/emptyPage.err\n"
			...
			Thread.start{
				...
				if(!(trainingInstance.genome_ftp_link == null)){
					...
					fileSizeScript << "curl -sI ${trainingInstance.genome_ftp_link} | grep Content-Length | cut -d ' ' -f 2 > ${projectDir}/genomeFileSize 2> ${projectDir}/genomeFileSize.err"
					...
					if(genome_size < maxFileSizeByWget){//1 GB
						...
						getGenomeScript << "wget -O ${projectDir}/genome.fa ${trainingInstance.genome_ftp_link} > ${projectDir}/getGenome.out 2> ${projectDir}/getGenome.err"
						...
						if("${trainingInstance.genome_ftp_link}" =~ /\.gz/){
							def gunzipGenomeScript = new File("${projectDir}/gunzipGenome.sh")
							gunzipGenomeScript << "cd ${projectDir}; mv genome.fa genome.fa.gz; gunzip genome.fa.gz 2> ${projectDir}/genome.gunzip.err"
							def gunzipGenome = "bash ${gunzipGenomeScript}".execute()
							gunzipGenome.waitFor()			
							//////delProc = "rm ${gunzipGenomeScript}".execute()
							//////delProc.waitFor()
...
"""
				...
				if(!(trainingInstance.est_ftp_link == null)){
					...
					def fileSizeScript = new File("${projectDir}/filzeSize.sh")
					fileSizeScript << "curl -sI ${trainingInstance.est_ftp_link} | grep Content-Length | cut -d ' ' -f 2 > ${projectDir}/estFileSize 2> ${projectDir}/estFileSize.err"
					def retrieveFileSize = "bash ${fileSizeScript}".execute()
					retrieveFileSize.waitFor()			
					def delSzCrProc = "rm ${fileSizeScript}".execute()
					delSzCrProc.waitFor()
					...
					if(est_size < maxFileSizeByWget){//1 GB
						...
						getEstScript << "wget -O ${projectDir}/est.fa ${trainingInstance.est_ftp_link} > ${projectDir}/wget.out 2> ${projectDir}/wget.err"
						def wgetEst = "bash ${projectDir}/getEstScript.sh".execute()
						wgetEst.waitFor()
						if("${trainingInstance.est_ftp_link}" =~ /\.gz/){
							def gunzipEstScript = new File("${projectDir}/gunzipEst.sh")
							gunzipEstScript << "cd ${projectDir}; mv est.fa est.fa.gz; gunzip est.fa.gz 2> ${projectDir}/est.gunzip.err"
...
"""
				...
				if(estExistsFlag == 1){
					...
					if(avEstLen < estMinLen){
						logFile << "${trainingInstance.accession_id} EST sequences are on average shorter than ${estMinLen}, suspect RNAseq raw data. Abort job.\n"
						...
						logFile << "${trainingInstance.accession_id} EST sequences are on average longer than ${estMaxLen}, suspect non EST/cDNA data. Abort job.\n"
				...
				if(!(trainingInstance.protein_ftp_link == null)){
					...
					def fileSizeScript = new File("${projectDir}/filzeSize.sh")
					fileSizeScript << "curl -sI ${trainingInstance.protein_ftp_link} | grep Content-Length | cut -d ' ' -f 2 > ${projectDir}/proteinFileSize 2> ${projectDir}/proteinFileSize.err"
					def retrieveFileSize = "bash ${fileSizeScript}".execute()
					retrieveFileSize.waitFor()			
					def delSzCrProc = "rm ${fileSizeScript}".execute()
					delSzCrProc.waitFor()
					...
					if(protein_size < maxFileSizeByWget){//1 GB
						...
						getProteinScript << "wget -O ${projectDir}/protein.fa ${trainingInstance.protein_ftp_link} > ${projectDir}/getProtein.out 2> ${projectDir}/getProtein.err"
						def wgetProtein = "bash ${projectDir}/getProtein.sh".execute()	
						wgetProtein.waitFor()
						if("${trainingInstance.protein_ftp_link}" =~ /\.gz/){
							def gunzipProteinScript = new File("${projectDir}/gunzipProtein.sh")
							gunzipProteinScript << "cd ${projectDir}; mv protein.fa protein.fa.gz; gunzip protein.fa.gz 2> ${projectDir}/protein.gunzip.err"	
...
"""
				...
				if(grepContent =~ /Struct-Cksum/){
					...
					oldAccScript << "grep \"Grails-ID: \\[${oldID}\\]\" ${dbFile} | perl -ne \"@t = split(/\\[/); @t2 = split(/\\]/, \\\$t[4]); print \\\$t2[0];\" > ${oldAccResult} 2> ${projectDir}/grepPrevAcc.err"
...
"""
				...
				if( estExistsFlag ==1 && proteinExistsFlag == 0 && structureExistsFlag == 0){
					...
					logFile << "${trainingInstance.accession_id} EST: ${estExistsFlag} Protein: ${proteinExistsFlag} Structure: ${structureExistsFlag} SGE-script remains empty! This an error that should not be possible.\n"
				}
				// write submission script
				def submissionScript = new File("${projectDir}/submitt.sh")
				def fileID = "${projectDir}/jobID"
				submissionScript << "cd ${projectDir}; /usr/bin/qsub augtrain.sh > ${fileID} 2> ${projectDir}/qsub.err"
				// submitt job
				...
				statusScript << "cd ${projectDir}; /usr/bin/qstat|grep augtrain |grep ${jobID} > ${statusFile} 2> ${projectDir}/qstat.err"
				def statusContent
				def statusCheck 
				def qstat = 1
				def runFlag = 0;
...
"""
					...
					packResults << "cd ${output_dir}; tar -czvf ${trainingInstance.accession_id}"
					...
					cleanUp.waitFor()
					def cleanUp = "rm ${output_dir}/pack${trainingInstance.accession_id}.sh".execute()
					packResults = new File("${output_dir}/pack${trainingInstance.accession_id}.sh")
					packResults << "; rm -r ${trainingInstance.accession_id};"
					cleanUp = "bash ${output_dir}/pack${trainingInstance.accession_id}.sh".execute()
					cleapUp.waitFor()
					cleanUp = "rm ${output_dir}/pack${trainingInstance.accession_id}.sh".execute()
					cleanUp.waitFor()
					logFile << "${trainingInstance.accession_id} autoAug directory was packed with tar/gz.\n"
...
"""
						...
						def packResults = new File("${output_dir}/pack${trainingInstance.accession_id}.sh")
						packResults << "cd ${output_dir}; tar -czvf ${trainingInstance.accession_id}; rm -r ${trainingInstance.accession_id};"
						//packResults << "cd ${output_dir}; tar cf - ${trainingInstance.accession_id} | 7z a -si ${trainingInstance.accession_id}.tar.7z; rm -r ${trainingInstance.accession_id};"
						def cleanUp = "bash ${output_dir}/pack${trainingInstance.accession_id}.sh".execute()
						cleanUp.waitFor()
