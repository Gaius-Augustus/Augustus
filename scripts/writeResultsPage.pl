...
my $genomeChkSum = $tdbLine[13];
my $genomeFileSize = $tdbLine[15];
my $estChkSum = $tdbLine[21];
my $estFileSize = $tdbLine[23];
my $proteinChkSum = $tdbLine[29];
my $proteinFileSize = $tdbLine[31];
my $structChkSum = $tdbLine[35];
my $structFileSize = $tdbLine[37];

## make sure that no weird zero characters were introduced by grails or perl
#while($projectID =~ m/0/){
#	$projectID =~ s/0/O/;
#}
