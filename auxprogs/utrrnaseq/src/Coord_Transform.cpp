...
Coord_Transform::Coord_Transform(unsigned comp_start, int dir, unsigned limit, unsigned max_pos, unsigned W,
    ...
    int skip_end = 0; //normally end position + W for this next intron
        ...
        if (dir == 1) { //left -> right
            if ((*curr_introns->begin()).strand == "+") {
                it = find_if(curr_introns->begin(), curr_introns->end(),
                             [i](Genomic_Data::Intron const& intron){return (int) intron.start >= i;});
            }
            else {
                it = find_if(curr_introns->begin(), curr_introns->end(),
                             [i](Genomic_Data::Intron  const& intron){return (int) intron.end > i;});
            }
        }
        else { //right -> left
