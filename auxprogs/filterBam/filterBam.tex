...
Here we describe how to install filterBam on a Linux terminal. 

\subsection{Requirements}
This software has been tested on a a Dell (x86\_64) computer with Ubuntu [version]. Compilation of the 
source code was done with GNU's C++ compiler, gcc version 4.4.3, with -std=c++0x experimental features 
enabled, that is now part of the c++11 implementation of C++ programming language. 
Table \ref{tab:software} below 
shows the list of required software packages for compiling filterBam, or that might be helpful to have 
when using filterBam. filterBam requires the zlib library \url{<http://www.zlib.net>} for support of the 
BGZF format and also of libbamtools.a, which is a library within the Bamtools distribution.
...
\begin{table}
  \begin{center}
    \begin{tabular}{|l|l|l|}
	\hline
      Software	& 	version	& Available at \\ \cline{1-3}
      Bamtools	&	2.1.0	& \url{https://github.com/pezmaster31/bamtools} \\ \cline{1-3}
      Samtools	& 	0.1.18	& \url{http://samtools.sourceforge.net/} \\ \cline{1-3}
	  Library	&  version  & \\ \cline{1-3}
	  zlib		&  $>=$ 1.2.2.1 & \\ \cline{1-3}
    \end{tabular}
  \end{center}
  \label{tab:software}
...
Type `make' to compile filterBam, the Makefile script will create directory 'build' where the binary 
will be stored, keeping it separate from the source files. Set the \$BAMTOOLS variable within the 
...
is 2.0 and is available on the website \url{https://github.com/pezmaster31/bamtools/downloads}. Bam
 According to the Bamtools porting guide (Barnett, October 2011), BT-2x and beyond will be neither binary- 
nor source-compatible with the earlier BT-0x or BT-1x versions. Your programs compiled using those classes 
will not work with the new libraries.
  ...
  \begin{center}
    \begin{tabular} {|c|c|c|c|} \hline
	 \multicolumn{4}{|c|}{\textbf{filterBam}} \\ \hline
      ...
      \multirow{4}{*}{Screens out} & unmapped & none &  none \\ \cline{2-4}
      ...
      \multirow{2}{*}{Screens out} & best & none & nore \\ \cline{2-4}
