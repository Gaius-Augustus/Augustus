 ...
 * @param alignments
 */
void BamToolsUtils::sortByQueryNameAscending(std::vector<BamAlignmentRecord_> &alignments) const {
    static BamTools::Algorithms::Sort::ByName ascNameSorter(BamTools::Algorithms::Sort::AscendingOrder);
    static BamAlignmentRecordSorter ascNameSorterFunc = [](const BamAlignmentRecord_ lx, const BamAlignmentRecord_ rx) {
        const BamToolsAlignmentRecord& barl = dynamic_cast<const BamToolsAlignmentRecord&> (*lx);
        const BamToolsAlignmentRecord& barr = dynamic_cast<const BamToolsAlignmentRecord&> (*rx);
        return ascNameSorter(*(barl.getAlignment()), *(barr.getAlignment()));
    };
 ...
 * @param alignments
 */
void BamToolsUtils::sortByPositionAscending(std::vector<BamAlignmentRecord_> &alignments) const {
    static BamTools::Algorithms::Sort::ByPosition ascPositionSorter(BamTools::Algorithms::Sort::AscendingOrder);
 ...
 * @param alignments
 */
void BamToolsUtils::sortByscTagDescending(std::vector<BamAlignmentRecord_> &alignments) const {
    ...
    // the use of template <float> is neither an option, as the getTagData function returns false for float values
